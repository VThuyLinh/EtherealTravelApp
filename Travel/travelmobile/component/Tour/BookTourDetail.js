...
const BookTourDetail = ({ navigation }) => {
    ...
    const loadBookTourDetail = useCallback(async () => { 
        setRefreshing(true);
        try {
            setLoading(true);
            ...
            console.log("[loadBookTourDetail] Data loaded successfully:", res.data.length, "bookings,", res1.data.results.length, "tours");
        } catch (ex) {
            console.error("[loadBookTourDetail] Error loading data:", ex);
        } finally {
            setRefreshing(false);
    ...
    const updateBookTourState = async (bookTourId, newState) => {
        ...
        try {
            ...
            const response = await axios.patch(`https://thuylinh.pythonanywhere.com/BookTour/${bookTourId}/`, formData, {
                headers: {
                    'Authorization': `Bearer ${token}`,
                    'Content-Type': 'application/json'
                },
            });
            console.log(`[updateBookTourState] Đã cập nhật BookTour ID ${bookTourId} sang trạng thái: ${newState}`);
            console.log("[updateBookTourState] Response:", response.data); // Log response data
            loadBookTourDetail(); // Sau khi cập nhật, tải lại dữ liệu
        } catch (error) {
            console.error(`[updateBookTourState] Lỗi khi cập nhật BookTour ID ${bookTourId}:`, error.request);
            console.error("[updateBookTourState] Error Response:", error.response?.data); // Log error response
        }
    };
        ...
        booktourdetail.forEach(btd => {
            if (btd.State === 'Wait for Paid') {
                ...
                if (tourInfo && tourInfo.DepartureDay && tourInfo.DepartureTime) {
                    ...
                    console.log(`[checkAndAutoCancel] Checking ID: ${btd.id}, Departure Time: ${departureTime}, Now: ${now}, Time Before: ${timeBeforeDeparture}`);
                    ...
                    if (timeBeforeDeparture <= 0 && timeBeforeDeparture > -30 * 60 * 1000) { // Hủy trong khoảng 48h trước và 30p sau giờ khởi hành
                        console.log(`[checkAndAutoCancel] Đã quá thời hạn thanh toán 48h cho BookTour ID: ${btd.id}. Thực hiện hủy.`);
        ...
        for (const btd of booktourdetail) {
            if (btd.State === 'Paid') {
                ...
                if (tourInfo && tourInfo.DepartureDay && tourInfo.DepartureTime) {
                    ...
                    console.log(`[completeBookTour] Checking ID: ${btd.id}, Now: ${now}, Departure: ${departureTime}, Complete At: ${completionTime}, Current State: ${btd.State}`);

                    if (now >= completionTime && btd.State !== 'Complete') {
                        console.log(`[completeBookTour] Đã quá 30 phút sau giờ khởi hành cho BookTour ID: ${btd.id}. Chuyển sang trạng thái Complete.`);
    ...
    const handlePaymentSuccess = (bookTourId) => {
        ...
        console.log(`[handlePaymentSuccess] Thanh toán thành công cho BookTour ID: ${bookTourId}. Tiến hành cập nhật trạng thái.`);
    ...
    useFocusEffect(
        useCallback(() => {
            ...
            const intervalIdComplete = setInterval(() => { // Thay đổi interval ở đây
                completeBookTour();
            }, 3 * 60 * 1000); // Kiểm tra mỗi 30 giây để hoàn thành
                ...
                {booktourdetail.map(c => c.id_customer_bt !== user.id ? null : (
                    <React.Fragment key={c.id}>
                        {booktourdetail === null ? (
                            ...
                            <Card mode="elevated" style={{ backgroundColor: "#f1faee", marginBottom: 30, marginLeft: 8, marginRight: 8, marginTop:10}}>
                                <Card.Content>
                                    ...
                                    {c.State === "Wait for Paid" ? (
                                        <>
                                            ...
                                            <View style={{ flexDirection: 'row' }}>
                                                ...
                                                <Button style={StyleTour.btn1b} onPress={()=>navigation.navigate("reject")}><Text style={StyleTour.text22}>Hủy</Text></Button>
                                            </View>
                                        </>
                                    ) : null}
