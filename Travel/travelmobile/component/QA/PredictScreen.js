...
const PredictScreen = ({ route }) => {
    ...
    try {
        ...
        if (response.data && typeof response.data.predicted_locations === 'string') {
            setPredictions([response.data.predicted_locations]);
            console.warn("result", response.data.predicted_locations);
            console.log("chuoi", [response.data.predicted_locations]);
        } else {
            setPredictions([]);
            console.warn("No valid predicted_locations found in response.");
        }
